...
> 状态:Phase 0 草案。常量的唯一权威是同目录 [`protocol.json`](./protocol.json),本文只解释语义;两者冲突以 json 为准。
> 最终归宿:`protocol.json` 入 scriptcat 仓库、镜像进 `scriptscat/sctl`(CI 逐字节比对);本文改写为 sctl 仓库 `PROTOCOL.md`(届时补英文版)。
> 设计背景与决策记录见 [`../2026-07-17-mcp-bridge-ws-redesign-design.md`](../2026-07-17-mcp-bridge-ws-redesign-design.md)。
...
- 写策略为「直接允许」(全局或该客户端覆盖)→ 不挂起,立即执行返回(源码披露不受此豁免;内建 `sctl-cli` 的源码读取例外,见设计文档 §3.1)。
...
- **威胁边界**:同用户完整权限的本机进程不设防(可读 daemon 密钥文件),见 THREAT-MODEL;
