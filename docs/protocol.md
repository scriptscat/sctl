...
The `pair.request` / `pair.decision` / `client.revoke` / `client.sync` envelope types remain in
`envelopeTypes` for protocol-version stability, but **neither the daemon nor the extension produces or handles
them** under the flat-trust model; a peer that receives one ignores it (the forward-compatible path of §2).
`McpClientRecord` and the per-client store are gone on both sides.
