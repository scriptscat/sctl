...
|---|---|---|---|
| `<dataDir>/pairing.key` | POSIX 0600; protected current-user DACL on Windows | The extension's long-term key K (hex), established by enrollment | Allows impersonating the extension when connecting to the daemon; still bound by write approval |
