// Package mcpidentity 持久化 sctl mcp 实例的已配对客户端身份。
//
// 背景(设计文档 §4 的被点名待定项):CLI 动词以内建 `sctl-cli` 身份、全量 scope、不配对运行;
// 但 MCP agent(Claude/Codex)按 §4 仍逐客户端配对(8 位码、铸造令牌、按 scope 过滤 tools/list、
// 可撤销)。矛盾在于 `sctl mcp` 的 stdout 被 MCP 协议独占,无法在其中交互展示配对码。
//
...
import (
	...
	"github.com/scriptscat/sctl/internal/fsutil"
)
...
func Load(name string) (*Identity, error) {
	raw, err := os.ReadFile(config.McpIdentityFile(name))
...
func Save(name string, id *Identity) error {
	path := config.McpIdentityFile(name)
